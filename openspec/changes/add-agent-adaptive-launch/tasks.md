...
- [ ] 5.1 Launcher: adaptive command uses `executable`/`resumeCommand` (not `--prompt`); ready-then-send delivers the prompt after a matching capture (fake tmux runner + fake capture); passthrough `command` launched verbatim
