...
- **WHEN** `POST /tasks/{id}/run` is called for a `new-session` task with no `targetRef` and a tmux session of the project's name already exists whose `@ccmux_project` does not match the task's project (a different project, or an unstamped session created outside ccmux)
