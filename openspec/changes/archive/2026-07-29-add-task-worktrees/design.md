...
- **Auto-adopting a non-wtm repo (`wtm init`).** Out of scope for this change — a worktree task in a non-wtm repo is blocked (task stays pending) and the dev self-inits with plain `wtm init`. A future change may add opt-in auto-init behind the guard documented under Open Questions.
...
Rationale: `wtm init` adopt is invasive — `convertToBare` moves the working tree into `<root>/<branch>`, refuses on a dirty tree, and disrupts any other pane/editor/shell already rooted in the repo. Putting that in ccmux's automatic run path would surprise the dev and could abort mid-flow. Keeping it a manual dev step (plain `wtm init` — **not** a wrapper that symlinks an agent-launching `post_create`; see D8) is the safe default for now.
...
### D8: ccmux owns the agent launch — `post_create` must not launch one
wtm runs each repo's `post_create` hook after `git worktree add` — **even under `--no-shell`** (verified: `--no-shell` only suppresses wtm's own interactive shell, not the hook). Since ccmux calls `wtm create` and *then* launches the agent itself (into its project-session/branch-window model, with correlation + resume), a `post_create` that also launches an agent would **double-launch** — one agent from the hook (in its own per-worktree session) and one from ccmux. Decision: ccmux owns the launch; a ccmux-driven repo's `post_create` must be setup-only (deps/env) or empty. The dev adopts with **plain `wtm init`** (whose default `post_create` is a harmless stub), **not** a shell wrapper that symlinks an agent-launching hook. ccmux cannot suppress `post_create` per-invocation (wtm exposes no such flag), so this is a repo-setup convention, enforced by documentation (the `tasks` skill) rather than code.
*Alternative considered:* have ccmux pass a marker env var (e.g. `CCMUX_TASK_WORKTREE=1`) that a hook checks to skip its launch, preserving an auto-launching hook for manual `wtm create`. Rejected for now as extra coordination for a workflow the dev is standardizing away from; revisit if manual-`wtm create` auto-launch is wanted alongside ccmux.
...
- **`wtm create` runs the repo's `post_create` hook, even under `--no-shell`** → an agent-launching hook double-launches against ccmux's own launch. Mitigation: the ccmux-driven repo's `post_create` must be setup-only/empty (see D8); ccmux does not add or interpret hooks, and cannot suppress them per-invocation.
...
- **Deferred: opt-in auto-init.** A later change could adopt a non-wtm repo automatically behind an explicit opt-in (`--wtm-init` flag or `worktree.autoInit` config), gated by a safety precheck: refuse unless the working tree is clean **and** ccmux sees no other live sessions/panes rooted in that repo (the adopt layout-move would disrupt them). It would run plain `wtm init` (whose `post_create` does not launch an agent — ccmux owns the launch, see D8), never a hook-linking wrapper. Interactive TUI could confirm inline; headless runs would require the explicit opt-in. Captured here so the block-now behavior can graduate safely.
