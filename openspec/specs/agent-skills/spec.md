...
The Agent Skills that ship with ccmux — packaged, trigger-described instruction sets that teach an agent to drive ccmux's interfaces. This capability covers the `tasks` skill that teaches the `ccmux task` CLI (scaffolding, running, resuming, and tracking units of agent work), its mirroring between the ccmux plugin and the repo's own `.claude/skills/` tree, and the plugin/marketplace metadata that advertises it.
