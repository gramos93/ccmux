...
Provisioning and resolving per-task git worktrees so a task can run in an isolated working copy on its own branch. Worktrees are managed exclusively through `wtm` (the bare-repo worktree manager), resolved lazily at task run/resume time, reused idempotently, and never auto-removed. This capability defines the worktree provider, its branch/base defaulting, the non-wtm refusal policy, and how worktrees group under their repository; the task-launch capability builds on it to place the launched agent into the resolved worktree.
